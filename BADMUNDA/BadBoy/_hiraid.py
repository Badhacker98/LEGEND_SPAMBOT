...
@Client.on_message(filters.user(sudos) & filters.command(["hraid"], prefixes=HANDLER))
...
@Client.on_message(
    filters.user(sudos) & filters.command(["hrraid", "hreplyraid"], prefixes=HANDLER)
)
async def rhiraid(Badmunda: Client, e: Message):
    global USERS
    try:
    ...
    if int(user.id) in USERS:
        return await e.reply_text("User already in Rhiraid list!")
    USERS.append(user.id)
    mention = user.mention
    await e.reply_text(f"Reply Rhiraid Activated On User {mention}")
    if LOG_CHANNEL:
        try:
            await Badmunda.send_message(
                LOG_CHANNEL,
                f"Activated Reply Rhiraid By User: {e.from_user.id} \n\n On User: {mention} \n Chat: {e.chat.id}",
            )
        except Exception as a:
            print(a)
...
@Client.on_message(
    filters.user(sudos) & filters.command(["draid", "dhraid", "dreplyraid"], prefixes=HANDLER)
)
async def dhiraid(Badmunda: Client, e: Message):
    global USERS
    try:
        lol = e.text.split(" ", 1)[1].split(" ", 1)
    except IndexError:
    ...
    if int(user.id) not in USERS:
        await e.reply_text("User not in Hiraid list!")
        return
    USERS.remove(user.id)
    mention = user.mention
    await e.reply_text(f"Reply Hiraid Deactivated Successfully On User {mention}")
    if LOG_CHANNEL:
        try:
            await Badmunda.send_message(
                LOG_CHANNEL,
                f" Deactivated Reply Hiraid By User: {e.from_user.id} \n\n User: {mention} \n Chat: {e.chat.id}",
            )
        except Exception as a:
            print(a)
...
async def rlist(Badmunda: Client, e: Message):
    global USERS
    _reply = "**Hiraid users list - Pb Bot Spam** \n\n"
    if len(USERS) > 0:
        for x in USERS:
            try:
...
async def checkraid(Badmunda: Client, msg: Message):
    ...
    if int(msg.from_user.id) in USERS:
        await msg.reply_text(choice(HIRAID))
